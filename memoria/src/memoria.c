...
void* atender_cpu(void* socket_cliente_ptr) {
   ...
   while (control_key){
    ...
    case RESIZE: // Parametros: PID, Bytes
        ...
        if(resultado >= 0){
            cargar_int_a_buffer(response_buffer, 1);
    ...
    case LEER: // Parametros: PID, Direccion Fisica, Cantidad bytes
        ...
        uint32_t bytes_leidos = leer_memoria(pid, direccion_fisica, cantidad_bytes);

        response_buffer = crear_buffer();
        cargar_uint32_a_buffer(response_buffer, bytes_leidos);
    ...
    case ESCRIBIR: // Parametros: PID, Direccion Fisica, bytes a escribir
        ...
        // TODO: mejorar envio de paquetes sin buffer.
        response_buffer = crear_buffer();
        cargar_int_a_buffer(response_buffer, 4);
