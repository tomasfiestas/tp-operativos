...
u_int32_t leer_memoria(int pid, int direccion_fisica, int bytes)
{
    ...
    memcpy(tamanio, &memoria_total[direccion_fisica], bytes);
    uint32_t valor = *tamanio;
    free(tamanio);
...
int escribir_memoria(int pid, int direccion_fisica, char *bytes){
    uint32_t* tamanio = malloc(sizeof(uint32_t));
    *tamanio = (uint32_t)strtoul(bytes, NULL, 10);
    
    memcpy(&memoria_total[direccion_fisica], tamanio, sizeof(uint32_t));
    log_info(memoria_logger, "PID: %d - Accion: ESCRIBIR - Direccion fisica: %d - Tamaño: %zu B", pid, direccion_fisica, sizeof(uint32_t));
    free(tamanio);
    return 1;
}
